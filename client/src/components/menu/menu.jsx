...
const Menu = ({data}) => {
    ...
    return (
        <div id='menu' className='relative'>
            ...
            <ul ref={menuItemsRef} className={`${dynamicStyles} bg-lightMode_toLightC dark:bg-darkMode_lightC opacity-0 scale-90 transition-transform duration-200 invisible absolute w-52 top-14 justify-between flex-col border dark:border-darkMode_toLightC shadow-2xl p-2 rounded-c`}>
                {data.items.map(v => 
